...
authEmitter.on("userRegistered", async ({ email, firstName, lastName, token }) => {
...
authEmitter.on("passwordResetRequested", async ({ email, firstName, lastName, token }) => {
  const allowedOrigins = process.env.ALLOWED_ORIGINS
    ? process.env.ALLOWED_ORIGINS.split(",")
    : ["http://localhost:5173"];
  const frontendUrl = allowedOrigins[0];
...
authEmitter.on("passwordResetSuccess", async ({ email, firstName, lastName }) => {
  const fullName = `${firstName} ${lastName}`.trim();
